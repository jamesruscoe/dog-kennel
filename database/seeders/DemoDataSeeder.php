...
class DemoDataSeeder extends Seeder
{
    ...
    {
        $staff = User::where('role', UserRole::Staff->value)->get();
        ...
        $owners = collect($ownerData)->map(function ($data) {
        ...
        $dogs = collect($dogProfiles)->map(function ($profile) use ($owners) {
        ...
        foreach ($bookingDefinitions as $def) {
            ...
            // ── Care logs for currently-boarding dogs ─────────────────────────
            if ($def['care_logs']) {
                $this->seedCareLogsForBooking($booking, $staffIds);
            }
        }
    }
     ...
     * @param  int[]  $staffIds
     */
    private function seedCareLogsForBooking(Booking $booking, array $staffIds): void
    {

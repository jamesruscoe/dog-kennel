...
onMounted(() => {
    ...
    // Mark as read
    router.post(tenantRoute('staff.messages.read', props.conversation.id), {}, { preserveScroll: true, preserveState: true });

    // Listen for real-time messages via Echo if available
...
function sendReply() {
    form.post(tenantRoute('staff.messages.reply', props.conversation.id), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset();
            // Refresh messages
...
const otherPartyName = props.conversation.owner_user?.name ?? 'Unknown';
</script>
...
<template>
    ...
    <PageHeader
        ...
        :breadcrumbs="[
            { label: 'Messages', href: tenantRoute('staff.messages.index') },
